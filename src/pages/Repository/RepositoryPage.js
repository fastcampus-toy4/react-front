...
const HeartIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="heart-icon"><path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" /></svg> );
// SVG 새로고침 아이콘 컴포넌트 추가
const RefreshIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="refresh-icon">
      <polyline points="23 4 23 10 17 10"></polyline>
      <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"></path>
    </svg>
);
const RepositoryCard = ({ item, isExpanded, onExpand, onLikeToggle }) => {
    return (
        <div className={`card repository-card ${isExpanded ? 'expanded' : ''} ${!item.isLiked ? 'deleting' : ''}`} onClick={onExpand}>
            <div className="card-header">
                <h3>{item.title}</h3>
...
function Repository() {
    ...
    const handleKeywordClick = useCallback(async (keyword) => {
        ...
        try {
            ...
            console.error("Error starting chat from keyword:", error);
            alert(`채팅을 시작하지 못했습니다: ${error.message}`);
        } finally {
            setIsCreatingChat(false);
        }
    }, [isCreatingChat, navigate]);
        ...
        <div className="repository-page">
            ...
            <div className="keyword-section">
                ...
                <div className="keyword-header">
                    ...
                    <button className="refresh-btn" onClick={handleRefreshKeywords} title="새로운 키워드 추천받기">
                      <RefreshIcon />
                    </button>
                </div>
            </div>
